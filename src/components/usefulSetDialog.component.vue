<template>
  <div class="usefulSetDialog">
    <el-dialog :visible.sync="dialogTableVisible" title="常用语列表" size="large" class="contentTableDialog" @open="pageNum=1;getContentList()" @close="close">
      <p class="buttonBox">
    ...
    <el-dialog :visible.sync="dialogContentVisible" title="编辑常用语" size="large" class="contentEditDialog" @close="clearSel">
      <el-form :model="contentForm" :rules="contentFormRule" ref="contentForm" label-width="80px" label-position="left">
...
export default {
  ...
  methods: {
    close(){
      this.$store.commit('setUsefulSetDialog',false);
    },

<template>
  <div class="travelReimbDetail clearfix">
   <el-table :data="flightPersonTable" :stripe="true" highlight-current-row   class="appTable" >
      <el-table-column property="reciptName" label="乘机人"  ></el-table-column>
      <el-table-column property="reciptSex" label="性别" >
      ...
      <el-table-column property="reciptCredentialsAccount" label="证件号码" ></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleInfo() {
      ...
      .then(res => {
        if (res.status == 0) {
            for(var i=0;i<res.data.length;i++){
              if(res.data[i].dictCode==list.docTicEmployees[0].typeId){
                this.typeName=res.data[i].dictName
              }
      ...
      //   })
      // }
      if (list.docTicEmployeeFlights.length != 0) {//表格2
        list.docTicEmployeeFlights.forEach(i => {
          var item = {
      ...
      if (list.docTicEmployeeRecipts.length != 0) {//表格1
        list.docTicEmployeeRecipts.forEach(i => {
          var item = {

  ...
  <div class="paymentApp">
    <el-form label-position="left" :model="budgetForm" :rules="budgetRule" ref="budgetForm" label-width="128px">
      ...
      <ul class="budgetInfo clearfix clearBoth" v-show="budgetInfo">
        <li>年度预算{{budgetInfo.budgetTotal}}元</li>
        <li>可用预算{{budgetInfo.budgetRemain}}元</li>
        <li>预算执行比例{{budgetInfo.budgetRate*100}}%</li>
      </ul>
    ...
    <el-form label-position="left" :model="paymentForm" :rules="paymentRule" ref="paymentForm" label-width="128px">
      ...
      <el-form-item label="合同" prop="contractAttach" class="clearBoth">
        <el-upload class="myUpload" :auto-upload="false" :action="baseURL+'/doc/uploadDocFile'" :data="{docTypeCode:'DOC_ADM_APPROVAL'}" :on-success="handleContractSuccess" :on-error="handleContractError" :on-change="handleContractChange" ref="contractUpload" :on-remove="handleContractRemove">
      ...
      <el-form-item label="上传发票" prop="invoiceAttach">
        <el-upload class="myUpload" :auto-upload="false" :action="baseURL+'/doc/uploadDocFile'" :data="{docTypeCode:'DOC_ADM_APPROVAL'}" :on-success="handleInvoiceSuccess" :on-error="handleInvoiceError" :on-change="handleInvoiceChange" ref="invoiceUpload" :on-remove="handleInvoiceRemove">
          <el-button size="small" type="primary">上传文件<i class="el-icon-upload el-icon--right"></i></el-button>
        </el-upload>
      </el-form-item>
      <el-form-item label="费用类型" prop="feeType" placeholder="" class="width60">
        <el-select v-model="paymentForm.feeType" style="width:100%" ref="contractType">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      paymentRule: {
        ...
        feeType: [{ required: true, trigger: 'blur', message: '请选费用类型' }],
  ...
  methods: {
    ...
    submitAll() {
      ...
      var feeType = this.feeTypes.find(i => i.dictCode == this.paymentForm.feeType);
      var finPayment = {
    ...
    addBudget() {
      this.$refs.budgetForm.validate((valid) => {
        if (valid) {
          ...
          var item = {
            "budgetDeptId": dep.budgetDeptId, //预算部门id
            "budgetDeptName": dep.budgetDeptName, //预算部门名
            "budgetItemId": dep.budgetItemId, //预算科目id
    ...
    getFeeTypes() {
      this.$http.post('/api/getDict', { dictCode: 'FIN05' })
        .then(res => {
          if (res.status == '0') {

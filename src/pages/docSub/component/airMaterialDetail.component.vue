<template>
  <div class="airRepairDetail clearfix">
    ...
    <p class="totalMoney">金额总计 人民币 <span>{{info[0].airmPos.rmb | toThousands}} 元 {{info[0].airmPos.rmb | moneyCh}}</span></p>
    <el-row style="border-top: 1px solid #D5DADF;margin-top:20px;">
      ...
      <el-col :span="12" class="rightBorder">
        ...
        <p v-if="info" class="textContent">{{info[0].airmPos.supplierBankAccountCode}}</p>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24" v-if="info&&info[0].airmPos.isAdvancePayment==1">        
        <h1 class="title">预付款百分比</h1>
        <p v-if="info" class="textContent">{{info[0].airmPos.advancePaymentPercent}}%</p>
      </el-col>
    </el-row>
  </div>
</template>

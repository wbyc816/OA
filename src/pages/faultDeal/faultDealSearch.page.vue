      ...
      <el-table :data="searchData" class="myTable" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" class-name="clickItem">
          <template scope="scope">
            ...
            <!-- <span class="cancelButton" @click.stop="deletefaultDeal([scope.row.id])">删除</span> -->
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
      ...
      handleSelectDeal(item) {
        // this.getEmpBankAccount(item.empId);
        // this.searchParams.faultDealEmpName=item.name;
    ...
    // },
    goDetail(row) {
      this.$router.push('/faultDeal/faultDealDetail/' + row.id)
    },

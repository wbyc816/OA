//获取部门列表
export const GET_DEPT_LIST = 'GET_DEPT_LIST'
//根据搜索条件搜索部门人员
export const QUERY_EMP_LIST = 'QUERY_EMP_LIST'
//设置搜索部门
export const SET_QUERY_DEP_ID = 'SET_QUERY_DEP_ID'
export const SET_QUERY_PAGE = 'SET_QUERY_PAGE'

export const GET_EMP_DATAIL = 'GET_EMP_DATAIL'
export const GET_EMERGENCY_CONTACT_INFO = 'GET_EMERGENCY_CONTACT_INFO'
export const UPDATE_EMERGENCY_CONTACT_INFO = 'UPDATE_EMERGENCY_CONTACT_INFO'
//修改个人信息
export const UPDATE_EMP = 'UPDATE_EMP'
export const SET_USER_INFO="SET_USER_INFO"
//改变搜索loading
export const SET_SEARCH_LOADING = 'SET_SEARCH_LOADING'
export const SET_SUBMIT_LOADING = 'SET_SUBMIT_LOADING'
//编辑状态
export const SET_EDIT_STATUS = 'SET_EDIT_STATUS'

//简历信息
export const GET_RESUME_INFO = 'GET_RESUME_INFO'
export const GET_EMP_POST_INFO = 'GET_EMP_POST_INFO'

//获取常量
export const GET_DICT = 'GET_DICT'
//密级程度
export const SET_CONFIDENT = 'SET_CONFIDENT'
//类型树
export const GET_TYPE_TREE = 'GET_TYPE_TREE'
//重要程度
export const SET_URGENCY = 'SET_URGENCY'
//公文接收人
export const SET_RECIVER = 'SET_RECIVER'
//附加公文
export const SELECT_DOC = 'SELECT_DOC'
//管理员状态
export const GET_ADMIN_STATUS = 'GET_ADMIN_STATUS'
//提交公文
export const SUBMIT_DOC = 'SUBMIT_DOC'
//获取公文类型
export const GET_DOC_FORM = 'GET_DOC_FORM'
//流程弹窗
export const SET_PROCESS_VIEW = 'SET_PROCESS_VIEW'
//流程数据
export const GET_PROCESS_DATA ="GET_PROCESS_DATA"
export const SET_AIR_PORT_LIST ="SET_AIR_PORT_LIST"


export const GET_ROOM_POSITION ="GET_ROOM_POSITION"

export const GET_CONFERENCE_TYPE ="GET_CONFERENCE_TYPE"

export const GET_CONFERENCE_NUM ="GET_CONFERENCE_NUM"

export const GET_DOC_TIPS ="GET_DOC_TIPS"
